...
backup = path.with_suffix(".tsx.bak-hero-try-buy-" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
def replace_function(source: str, function_name: str, replacement: str) -> str:
    ...
    if start == -1:
      raise SystemExit(f"Impossible de trouver la fonction {function_name}.")

    brace_start = source.find("{", start)

    if brace_start == -1:
      raise SystemExit(f"Impossible de trouver le début de {function_name}.")
    ...
    if end is None:
        raise SystemExit(f"Impossible de trouver la fin de {function_name}.")

    return source[:start] + replacement.strip() + "\n" + source[end:]

text = replace_function(text, "HeroSection", new_hero)

path.write_text(text, encoding="utf-8")

print("✅ Bandeau boutique modifié avec Try & Buy")
