...
backup = path.with_suffix(".tsx.bak-cta-final-" + datetime.now().strftime("%Y%m%d-%H%M%S"))
backup.write_text(text, encoding="utf-8")

component = r'''
function FloatingNeedHelpCTA() {
  const storageKey = "ecoliz_need_help_cta_hidden_v2";
  ...
  return (
    <div className="fixed bottom-5 left-4 right-4 z-[9999] sm:left-auto sm:right-6 sm:w-[430px]">
      ...
      <div className="relative overflow-hidden rounded-[1.3rem] border border-cyan-100/80 bg-gradient-to-br from-sky-950 via-cyan-950 to-sky-900 text-white shadow-[0_22px_70px_rgba(8,47,73,0.52)]">
        ...
        <button
          ...
          className="absolute right-3 top-3 z-20 flex h-7 w-7 items-center justify-center rounded-full border border-white/25 bg-white/10 text-sm font-black text-white transition hover:bg-white hover:text-sky-950"
...
def remove_function(source: str, function_name: str) -> str:
    while True:
        ...
        if end is None:
            ...
            positions = [source.find(marker, start + 1) for marker in markers]
...
if marker not in text:
    raise SystemExit("Impossible de trouver où insérer le nouveau CTA.")
